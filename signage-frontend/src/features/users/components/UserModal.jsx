...
const UserModal = ({ isOpen, onClose }) => {
  ...
  const handleAvatarSelect = (i) => {
    setFormData((prev) => ({
      ...prev,
      avatar: `/Digital-Signage-System/assets/avatars/avatar${i}.png`,
    }));
  };
...
const handleSubmit = async (e) => {
  ...
  style={{
    backgroundImage: "url('/Digital-Signage-System/assets/starry-background.jpg')",
  }}
       >
        <button
...
<form onSubmit={handleSubmit} className="grid grid-cols-2 md:grid-cols-2 gap-4 px-6">
...
<div className="col-span-2 mt-4">
  ...
  <div className="flex space-x-3">
    {[1, 2, 3, 4, 5, 6].map((i) => {
      const path = `/Digital-Signage-System/assets/avatars/avatar${i}.png`;
      return (
        <img
          key={i}
          src={path}
          ...
          className={`w-10 h-10 rounded-full cursor-pointer border-2 ${
            formData.avatar === path ? "border-blue-500" : "border-transparent"
          }`}
        />
      );
    })}
  ...
  <div className="col-span-2 flex justify-end mt-6 mb-8">
    <button type="submit" className="bg-blue-600 px-6 py-2 rounded-2xl hover:bg-blue-700">
      Save
    </button>
  </div>
</form>
